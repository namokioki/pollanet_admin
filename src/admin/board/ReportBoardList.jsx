...
import { Card, Form, Button, Table, Col, Badge } from 'react-bootstrap';
...
const ReportBoardList = ({ location,history }) => {
    ...
    const callBoard =async()=>{
      const resulte = await axios.get(`/board/list?search=${word}&b_category=2&page=${page}&num=${num}`)
  ...
  const onKeyDown = (e) => {
    if (e.keyCode === 13) {
      history.push(`/admin/board/reportlist?search=${e.target.value}&b_category=2&page=${page}&num=${num}`)
    }
    history.push(`/admin/board/reportlist?search=${e.target.value}&b_category=2&page=${page}&num=${num}`)
  }

  const onChangePage = (e) => {
    history.push(`/admin/board/reportlist?search=${word}&b_category=2&page=${e}&num=${num}`);
  }
...
export default withRouter (ReportBoardList)