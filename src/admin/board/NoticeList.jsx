...
const NoticeList = ({ location,history }) => {
  const search = qs.parse(location.search, { ignoreQueryPrefix: true })
   ...
   const [word, setWord]= useState(searchWord);
    ...
    const callBoard =async()=>{
      const resulte = await axios.get(`/board/list?search=${word}&b_category=4&page=${page}&num=${num}`)
  ...
  const onKeyDown = (e) => {
    if (e.keyCode === 13) {
      history.push(`/admin/board/noticeList?search=${e.target.value}&b_category=4&page=${page}&num=${num}`)
    }
    history.push(`/admin/board/noticeList?search=${e.target.value}&b_category=4&page=${page}&num=${num}`)
  }

  const onChangePage = (e) => {
    history.push(`/admin/board/noticeList?search=${word}&b_category=4&page=${e}&num=${num}`);
  }
